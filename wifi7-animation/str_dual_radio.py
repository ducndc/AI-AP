...
class STRDualRadio(Scene):
    def construct(self):
        ...
        # ==== Phase B: ICF (non-HT duplicate, 1 SS) on BOTH links (simultaneous) ====
        icf1 = pkt("ICF (non-HT dup, 1SS)", ORANGE).move_to(ap.get_right()+UP*1.2)
        icf2 = pkt("ICF (non-HT dup, 1SS)", ORANGE).move_to(ap.get_right()+DOWN*1.2)
        self.play(
            # cả hai ICF chạy đồng thời AP->STA
            run_packet(icf1, path1_AP2STA, t=4.4),
            run_packet(icf2, path2_AP2STA, t=4.4)
        )
        ...
        self.play(
            AnimationGroup(
                run_packet(ppdu_ul, path1_STA2AP, t=1.8),
                run_packet(ppdu_dl, path2_AP2STA, t=1.8),
        ...
        self.play(
            AnimationGroup(
                run_packet(ack_ul,  path1_AP2STA, t=1.2),
                run_packet(ack_dl,  path2_STA2AP, t=1.2),
                lag_ratio=0.0
            )
        )
